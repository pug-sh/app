  ...
  return (
    <ChartContext.Provider value={{ config }}>
      <div
        ...
        style={Object.entries(config).reduce<React.CSSProperties>((acc, [key, item]) => {
...
>(
  (
    { active, payload, className, label, labelFormatter, formatter, hideLabel = false },
    ref
  ) => {
    ...
    return (
      <div
        ref={ref}
        className={cn('grid min-w-[8rem] items-start gap-1.5 rounded-lg border border-border/50 bg-popover px-2.5 py-1.5 text-xs shadow-xl', className)}
      >
