...
export function mapOAuthConnectError(error: unknown, displayName: string, fallback: string) {
  ...
  if (reason === OAUTH_REASON_INVALID_TOKEN) return `Invalid or expired ${displayName} sign-in. Try again.`
  if (reason === OAUTH_REASON_PROVIDER_DISABLED) {
    ...
    return `${displayName} sign-in is temporarily unavailable.`
  }
