...
const buildCollection = (povGeometries: Record<string, Geometry>): FeatureCollection<Geometry> => ({
  ...
  features: collection.features.map((f): Feature<Geometry> => {
    ...
    return {
      ...f,
      id: Number(m49),
      geometry: cutAntimeridian(povGeometries[m49] ?? f.geometry),
...
// India, so we swap in patched IN/PK/CN geometries (keyed by M49 code) generated by the pug-maps
// repo and served alongside the basemap tiles. If the patch fails to load we fall back to the
// unpatched shapes — a degraded map beats no map.
