...
The palette was re-based on the `@bklit` chart registry's own theme (the vendored charts' upstream) — see `git log src/index.css`. What that pass established, and the traps it left:

- **Depth is tonal range, not brightness — and the range lives in the ink, not the canvas.** `--foreground` rose to `0.965` while `--muted-foreground` deliberately **dropped** to `0.70`, so body sits at ≈ 16:1 over muted's ≈ 6.6:1 where the old theme was a flat 11.9 over 7.2. Because the range is carried by the ink, the canvas is free to be comfortable: it went `0.26` → `0.155` → **`0.21`** (0.155 was tried and read as too dark). Don't lift `--muted-foreground` back toward the body colour to "fix" its contrast — that re-flattens the hierarchy and undoes the pass. It clears AA everywhere it lands (6.6:1 canvas, 6.2:1 card, 5.8:1 popover, 5.3:1 on `--muted`).
- Dark `--primary` (`0.54/0.16`) and `--destructive` (`0.54/0.18`) are deep + saturated so **white button text clears AA** (≈ 4.9:1 on the fill), matching the light-mode CTA character.
...
- **Dark `--border` is translucent, `--input` is solid — that asymmetry is load-bearing.** `--border` (`oklch(0.62 0.02 265 / 0.2)`) takes the tint of whatever surface it sits on, so one token works on canvas, card and popover. `--input` cannot do the same: every form control styles itself `dark:bg-input/30`, and Tailwind's `/NN` modifier *multiplies* alpha, so an alpha token there resolves to ≈`0.06` and the fill disappears. The `0.2` on `--border` is likewise picked for the modifier — 48 call sites dim it with `border-border/30..70`, and at `0.18` those thinned to ~1.05:1.
- Light mode was already at contrast parity with the registry (body 16.4:1 vs their 18.1:1) and only moved enough to let `--card` (now true white) separate from a canvas dropped to `0.972`. `--muted-foreground` had to go to `0.53`: at the old `0.55` the dropped canvas put it at 4.47:1, just under AA.
- Still deferred — **deliberate, not a bug:** `text-destructive` *text* (≈ 3.2:1, improved from ~2.8:1 by the deeper canvas) is sub-AA (the same fill-vs-text conflict). The fix is the same pattern (a `--destructive`-text token mirroring `--link`); not yet done because error text is rare. Don't lighten `--destructive` itself.
