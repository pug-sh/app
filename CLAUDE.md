...
Trends area, line and bar all render through it (`insights/charts/{area,line,bar}-chart.tsx`); the funnel is the last recharts chart.
...
**Upgrading is not just re-running the add.** Adding `@bklit/composed-chart` reverted every patch below at once and rewrote three shared files. Before any add:

- **Back up untracked vendored files.** The add rewrites files its registry entry never lists — `composed-chart` rewrote `line-chart.tsx` — and git cannot restore a file it isn't tracking.
- **`git diff` the whole tree afterwards, not just the component's directory.** Known collateral: `src/lib/utils.ts` (quote style, and `cn` from an arrow to a `function` declaration) and `src/index.css`, which gets re-broken the same two ways every time — three `--chart-line-primary: var(----chart-line-primary)` mappings with a doubled prefix that silently kill every utility built on them, and a `.dark` block re-pinning `--chart-background`/`--chart-grid`/`--chart-label` to the registry's own darks. Those are deliberately aliased in `:root` so they track the theme; the registry's values were authored against a much darker canvas and leave the grid darker than the surface it's drawn on. Both hunks are pure regressions — revert the file.
- **Diff each reverted patch before restoring.** Every one so far has been a clean revert with no upstream changes, which makes `git checkout` the right restore — but confirm that rather than assume it.

**Two patches sit on top of upstream. Re-apply after every upgrade:**
...
2. An optional `formatDateLabel?: (date: Date) => string` prop, threaded through **every** chart entry point — currently `area-chart.tsx`, `line-chart.tsx` and `composed-chart.tsx` → `time-series-chart-shell.tsx`, where it overrides the `dateLabels` memo. Each entry point carries its own copy of the plumbing, so a fourth chart needs patching too. Upstream formats with `shortDateFmt` (`Intl.DateTimeFormat("en-US", { month: "short", day: "numeric" })`) — browser-local and granularity-blind — and computes the labels internally, so there is no prop-level fix. Bucket labels must render in the project's reporting zone to match the server's bucket boundaries, and must vary by granularity or hour buckets all read `Jul 19`. **This one still compiles and still renders when reverted**, silently regressing to browser-local dates. `insights/charts/vendored-date-labels.test.tsx` is the check: it covers all three charts and fails per-chart, so a red bar test alone localises the miss to `composed-chart.tsx`. It stubs `@visx/responsive` because happy-dom reports the container as 0×0 — without a size the chart renders nothing and every assertion passes vacuously.
