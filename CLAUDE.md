...
- Surfaces: `settings/billing` (the page), `components/billing/usage-meter.tsx` (sidebar footer), `components/billing/usage-banner.tsx` (above the header, beside `DemoBanner`). All three read `billingAtom`, so mounting them in one commit is still one request; the atom fetches `GetBillingStatus` and `GetUsage` together and passes a one-day range to the latter, since `range` bounds the daily series only while `used_events` always covers the period.
