...
Plan, quota and checkout. **Dodo Payments** is the provider, and the dashboard never sees it except as two URLs the server mints — `CreateCheckoutSession` and `CreatePortalSession` both return a link and nothing else. There is no product id on the wire, no price the frontend owns, and no plan catalog in this repo: `ListPlans` serves it, because mirroring it here would put a second authority on what a plan costs.
