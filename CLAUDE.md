...
The SDK is installed from the **`dist` branch** of its GitHub repo, not npm — it carries unreleased work, and bun never runs `prepare` on a git install, so a plain `github:` spec would extract sources with no `dist/`. `bun.lock` pins the branch by commit sha; `bun update @pug-sh/browser` is what moves it. **The build has to match pug.sh's, the same way the project ID does:** the stored identity envelope changed shape after npm 0.0.4, and neither build can read the other's, so until both sides run the same one a visitor crossing between them thrashes their anonymous ID on the shared `.pug.sh` cookie.
